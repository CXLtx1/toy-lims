...
def list_quant():
    ...
    if keyword:
        like = f"%{keyword}%"
        conditions.append("""(xa.sample_name ILIKE ? OR xa.method ILIKE ? OR xa.batch ILIKE ?
            OR xa.external_id ILIKE ? OR s.name ILIKE ? OR s.lims_no ILIKE ?)""")
        params.extend([like] * 6)
    if sample_id:
        conditions.append("xa.sample_id=?")
    ...
    rows = db.execute(f"""SELECT xa.id,xa.external_id,xa.sample_id,xa.sample_name,xa.method,
        ...
        ORDER BY xa.analyzed_at DESC NULLS LAST, xa.id DESC LIMIT ? OFFSET ?""",
        params + [per_page, (page - 1) * per_page]).fetchall()

    items = []
    for row in rows:
        values = db.execute("""SELECT name,value,use_report,alt_name FROM xrf_values
            WHERE analysis_id=? ORDER BY value DESC""", (row["id"],)).fetchall()
        items.append({
            **dict(row), "kind": "quant",
...
def quant_detail(aid):
    ...
    row = db.execute("""SELECT xa.id,xa.external_id,xa.sample_id,xa.sample_name,xa.method,
        ...
        WHERE xa.id=? AND COALESCE(xa.kind,'quant')='quant'""", (aid,)).fetchone()
    if row is None:
        return jsonify(ok=False, error="扫描不存在"), 404
    values = [dict(v) for v in db.execute("""SELECT name,value,use_report,alt_name
        FROM xrf_values WHERE analysis_id=? ORDER BY value DESC""", (aid,))]
...
def list_uq():
    ...
    if keyword:
        like = f"%{keyword}%"
        conditions.append("""(xa.sample_name ILIKE ? OR xa.method ILIKE ? OR xa.external_id ILIKE ?
            OR s.name ILIKE ? OR s.lims_no ILIKE ?)""")
...
def load_uq_detail(db, aid):
    ...
    detail = db.execute("SELECT * FROM uq_analyses WHERE external_id=?",
    ...
    if detail is not None:
        ...
        channels = [dict(c) for c in db.execute("""SELECT name,int_cps,conc,sigma_conc,std_err,
            ...
            WHERE uq_analysis_id=? ORDER BY conc DESC NULLS LAST""", (detail["id"],))]
