...
class TestOverview(PostgresCase):
    def setUp(self):
        ...
        self.run_script("""
            CREATE TABLE samples (
                id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, name TEXT, lims_no TEXT,
                category TEXT DEFAULT '', is_liquid INTEGER DEFAULT 0,
                is_water_quality INTEGER DEFAULT 0, workflow_type TEXT DEFAULT 'regular',
                xrf INTEGER DEFAULT 0, status TEXT DEFAULT 'received', created_at TEXT,
                customer TEXT DEFAULT '', analysis_date TEXT DEFAULT '',
                analyst TEXT DEFAULT '', reviewer TEXT DEFAULT '');
            ...
            CREATE TABLE sample_analytes (
                ...
                analyte_id INTEGER DEFAULT 1, instrument_id INTEGER, status TEXT DEFAULT 'pending');
            ...
            CREATE TABLE xrf_analyses (
                id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, sample_id INTEGER, kind TEXT);
            CREATE TABLE result_order_templates (
                id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, items_json TEXT,
                is_default INTEGER);
            CREATE TABLE audit_logs (
                id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, created_at TEXT,
                username TEXT, action TEXT, entity_type TEXT, entity_id TEXT, reason TEXT,
                before_json TEXT, after_json TEXT, ip_address TEXT, terminal_name TEXT);
    ...
    def seed_workload(self):
        self.db.executemany("INSERT INTO instruments VALUES (%s,%s,%s,%s)", [
            (1, "ICP", "ppm", 2), (2, "Balance", "percent", 1), (3, "XRF", "xrf", 3),
        ])
        self.db.executemany("INSERT INTO samples(id,name,status,xrf) VALUES (%s,%s,%s,%s)", [
        ...
        self.db.executemany("INSERT INTO readings(sample_analyte_id,raw) VALUES (%s,%s)",
                            [(task, value) for task in range(1, 18) for value in range(4)])
        self.db.executemany("INSERT INTO xrf_analyses(sample_id,kind) VALUES (%s,%s)", [
    ...
    def test_audit_keyset_and_safe_exact_fields(self):
        self.db.executemany("""INSERT INTO audit_logs VALUES
            (%s,NULL,'operator','create','sample',%s,NULL,'secret-before','secret-after','private-ip','terminal')""",
    ...
    def test_audit_default_limit_cap_and_exact_page_boundary(self):
        self.db.executemany("INSERT INTO audit_logs(id,action,entity_type) VALUES (%s,'update','sample')",
    ...
    def test_sample_filter_and_batched_enrichment_preserve_list(self):
        ...
        single_count = len(self.counter.statements)
        self.counter.statements.clear()
        self.get("/api/samples", per_page=100)
        self.assertEqual(len(self.counter.statements), single_count)
