...
from pg_case import PostgresCase


class TestAuditContext(PostgresCase):
    def setUp(self):
        self.provision_database()
        self.db = self.connect()
        self.addCleanup(self.db.close)
        self.counter = self.connect(counting=True)
        self.addCleanup(self.counter._inner.close)
        self.run_script("""
            CREATE TABLE audit_logs (
                id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, created_at TEXT,
                username TEXT, action TEXT, entity_type TEXT, entity_id TEXT,
                before_json TEXT, after_json TEXT, reason TEXT, ip_address TEXT,
                terminal_name TEXT);
            CREATE TABLE samples (
                id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, name TEXT,
                lims_no TEXT, status TEXT);
            CREATE TABLE sample_analytes (
                id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, sample_id INTEGER,
                analyte_id INTEGER, preparation_id INTEGER, instrument_id INTEGER,
                method_id INTEGER, status TEXT);
            CREATE TABLE analytes (id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, name TEXT);
            CREATE TABLE preparations (
                id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, sample_id INTEGER,
                name TEXT, mass_g DOUBLE PRECISION, volume_ml DOUBLE PRECISION);
            CREATE TABLE instruments (
                id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, name TEXT, itype TEXT);
            CREATE TABLE methods (
                id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, name TEXT,
                formula TEXT, constants TEXT);
            CREATE TABLE results (
                sample_analyte_id INTEGER PRIMARY KEY, aux TEXT);
            CREATE TABLE readings (
                id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
                sample_analyte_id INTEGER, raw DOUBLE PRECISION,
                extra TEXT DEFAULT '{}', use_avg INTEGER DEFAULT 1, is_final INTEGER DEFAULT 0);
            CREATE TABLE xrf_analyses (
                id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, sample_id INTEGER);
            CREATE TABLE xrf_values (
                id INTEGER PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, analysis_id INTEGER);
            ...
            INSERT INTO results VALUES (20,'{"use":true,"expected":10,"measured":5}');
    ...
    def event(self, before=None, after=None, entity_id="101", entity_type="reading", action="update"):
        ...
        self.db.execute("""INSERT INTO audit_logs VALUES
            (1,'2026-09-08 10:00:00','operator',%s,%s,%s,%s,%s,'private-reason','private-ip','terminal')""",
    ...
    def test_exact_payload_zero_null_and_correct_parallel_reading(self):
        ...
        self.assertEqual(set(target), {"id", "sample_id", "analyte", "prep_name", "instrument", "itype",
                                       ...
                                       "aux", "status", "readings"})
        self.assertEqual(target["aux"], '{"use": true, "expected": 10, "measured": 5}')
    ...
    def test_current_zero_and_blank_are_available(self):
        for raw in (0, None):
            with self.subTest(raw=raw):
                self.db.execute("UPDATE readings SET raw=%s WHERE id=101", (raw,))
        ...
        for bad in (None, "", "{", "null", "[]", '"text"', "false", "[" * 1100):
            for column in ("before_json", "after_json"):
                with self.subTest(bad=str(bad)[:20], column=column):
                    self.event()
                    self.db.execute(f"UPDATE audit_logs SET {column}=%s WHERE id=1", (bad,))
    ...
    def test_reassigned_target_refuses_false_context(self):
        for parent in (10, 22, 999):
            self.db.execute("UPDATE readings SET sample_analyte_id=%s WHERE id=101", (parent,))
    ...
    def test_current_instrument_must_be_known_plain_raw_type(self):
        for itype in ("function", "xrf", "", None, "unknown", 'ppm" onmouseover="bad'):
            self.db.execute("UPDATE instruments SET itype=%s WHERE id=1", (itype,))
    ...
    def test_neighbors_boundaries_null_prep_and_readings_are_batched(self):
        self.counter.statements.clear()
        audit_context.load_context(self.counter, 1)
        select_count = sum(sql.lstrip().upper().startswith("SELECT") for sql in self.counter.statements)
        reading_batches = [sql for sql in self.counter.statements
                           if "FROM readings WHERE sample_analyte_id IN" in sql]
        ...
        self.counter.statements.clear()
        data = audit_context.load_context(self.counter, 1)
        self.assertEqual([row["id"] for row in data["rows"]], [20, 30, 40])
        self.assertEqual(sum(sql.lstrip().upper().startswith("SELECT") for sql in self.counter.statements),
                         select_count)
        self.db.execute("DELETE FROM sample_analytes WHERE id!=20")
        self.counter.statements.clear()
        self.assertEqual(len(audit_context.load_context(self.counter, 1)["rows"]), 1)
        self.assertEqual(sum(sql.lstrip().upper().startswith("SELECT") for sql in self.counter.statements),
                         select_count)

    def test_current_raw_and_json_fields_are_sanitized(self):
        attack = '\" autofocus onfocus=alert(1) x=\"'
        self.db.execute("UPDATE readings SET raw=%s,extra=%s WHERE id=101",
                        (attack, '{"V":"secret","m":2,"bad":NaN}'))
        self.db.execute("UPDATE results SET aux=%s WHERE sample_analyte_id=20",
                        (json.dumps({"use": True, "expected": attack, "measured": 2, "secret": "hidden"}),))
        data = self.load()
        self.assertEqual(data["values"]["current"], {"available": False, "value": None})
        target = data["rows"][1]
    ...
    def test_malformed_json_fields_do_not_crash_renderer(self):
        ...
        for value in (None, "{", "null", "[]", '"text"', "false", "[" * 1100):
            with self.subTest(value=str(value)[:20]):
                self.db.execute("UPDATE methods SET constants=%s", (value,))
                self.db.execute("UPDATE readings SET extra=%s", (value,))
                self.db.execute("UPDATE results SET aux=%s", (value,))
    ...
    def test_load_and_http_are_read_only_and_get_only(self):
        self.counter.statements.clear()
        self.assertTrue(audit_context.load_context(self.counter, 1)["supported"])
        ...
        self.assertTrue(all(sql.lstrip().upper().startswith("SELECT")
                            for sql in self.counter.statements))
    ...
    def test_real_scene_template_uses_shared_readonly_renderer(self):
        response = self.client.get("/api/audits/1/scene?mode=before")
        self.assertEqual(response.status_code, 200)
        html = response.get_data(as_text=True)
        self.assertIn("LabFlow 原始数据录入", html)
        self.assertIn('/labflow-preview/style.css', html)
        self.assertIn('/labflow-preview/entry-renderer.js', html)
        self.assertIn('entryRowHtml(row, 1, { readOnly: true })', html)
        self.assertIn('data-mode="before"', html)
        self.assertIn('scene.locator.reading_id', html)
        self.assertNotIn("private-ip", html)
        self.assertNotIn("private-reason", html)
    ...
    def test_public_payload_omits_snapshots_secrets_and_can_be_safely_embedded(self):
        ...
        self.db.execute("UPDATE samples SET name=%s WHERE id=1", (attack,))
        self.db.execute("UPDATE analytes SET name=%s", (attack,))
    ...
    def test_sample_audit_preserves_fields_and_adds_structured_locators(self):
        self.db.executemany(
            "INSERT INTO audit_logs(id,entity_type,entity_id,action) VALUES (%s,'reading',%s,'update')",
